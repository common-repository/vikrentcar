...
<style>
	.vrc-overrides-manager .overrides-body .override-file {
		display: flex;
		align-items: center;
		padding: 10px;
		border-bottom: 1px solid #ccc;
	}
	.vrc-overrides-manager .overrides-body .override-file #override-options {
		font-size: 16px;
		margin-right: 10px;
	}
	.vrc-overrides-manager .overrides-body .override-file input {
		flex: 1;
		margin-right: 4px;
	}

	.vrc-overrides-manager .overrides-body .overrides-editor[data-layout="columns"] {
		display: flex;
		flex-wrap: wrap;
	}
	.vrc-overrides-manager .overrides-body .overrides-editor[data-layout="columns"] > * {
	...
	.vrc-overrides-manager .overrides-body .overrides-editor[data-layout="columns"] > *:only-of-type {
		width: 100%;
	}

	.vrc-overrides-manager .overrides-body .overrides-editor .overrides-editor-default {
		visibility: hidden;
		position: fixed;
		transform: translateX(200%);
	}
	.vrc-overrides-manager .overrides-body .overrides-editor[data-layout="columns"] .overrides-editor-default,
	.vrc-overrides-manager .overrides-body .overrides-editor[data-layout="rows"] .overrides-editor-default {
		visibility: visible;
		position: initial;
		transform: none;
	}
	...
	.vrc-overrides-manager .overrides-body .CodeMirror {
		height: 100%;
	}
</style>
	...
	(function($) {
		...
		$(function() {
			...
			$('#override-copy').on('click', function() {
				...
				VRCCore.copyToClipboard($(this).prev('input')).then(() => {
					// display successful message on copy
					VRCToast.dispatch(new VRCToastMessage({
						title: '<?php echo addslashes(__('Copied!')); ?>',
						status: VRCToast.SUCCESS_STATUS,
						delay: 'auto',
					}));
				}).catch(() => {
					// cannot copy
			...
			Joomla.submitbutton = (task) => {
				if (task == 'override.save') {
					...
					// save code
					doAjax(
						'admin-ajax.php?action=vikrentcar&task=override.save',
						postData,
						(resp) => {
						...
						(error) => {
							...
							VRCToast.dispatch(new VRCToastMessage({
								body: error.responseText || '<?php echo addslashes(__('Connection Lost! Please try again.', 'vikrentcar')); ?>',
								status: VRCToast.ERROR_STATUS,
							}));
			...
			$('#override-options').vrcContextMenu({
				clickable: true,
				buttons: [
					// HIDE ORIGINAL CODE
					{
						state: 1,
						text: '<?php echo addslashes(__('Show files tree', 'vikrentcar')); ?>',
						separator: true,
					...
					// HIDE ORIGINAL CODE
					{
						text: '<?php echo addslashes(__('Hide original file', 'vikrentcar')); ?>',
					...
					// SHOW IN COLUMNS
					{
						text: '<?php echo addslashes(__('Show both in columns', 'vikrentcar')); ?>',
						action: saveOverrideLayoutMode,
						icon: isLayoutModeSelected,
						layoutMode: 'columns',
					},
					// SHOW IN ROWS
					{
						text: '<?php echo addslashes(__('Show one over the other', 'vikrentcar')); ?>',
						action: saveOverrideLayoutMode,
						icon: isLayoutModeSelected,
						layoutMode: 'rows',
						separator: true,
					...
					// DELETE
					{
						...
						action: (root, event) => {
							...
							setTimeout(() => {
								...
								const r = confirm(Joomla.JText._('VRC_WANT_PROCEED'));
