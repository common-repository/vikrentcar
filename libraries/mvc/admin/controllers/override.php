...
/**
 * VikRentCar plugin Override controller.
 *
 * @since 1.4.0
 */
class VikRentCarControllerOverride extends JControllerAdmin
{
	/**
	...
	public function save()
	{
		...
		if (!JSession::checkToken())
		{
			...
			{
				// ajax request
				VRCHttpDocument::getInstance($app)->close(403, JText::translate('JINVALID_TOKEN'));
			}
			else
			{
				// post request
		...
		// build return URL
		$url = sprintf(
			'admin.php?page=vikrentcar&view=overrides&client=%s&selectedfile=%s&overridefile=%s',
			$client,
			$file,
			$override
		);
		...
		if (empty($data['code']))
		{
			$error = __('No code received', 'vikrentcar');

			// missing code data
			if ($ajax)
			{
				// ajax request
				VRCHttpDocument::getInstance($app)->close(400, $error);
			}
			else
			{
		...
		$overridesModel = $this->getModel('overrides', 'VikRentCarModel');
		...
		if (!$supported)
		{
			$error = sprintf(__('Cannot use the file as destination: [%s]', 'vikrentcar'), $data['file']);
		...
		if (!$this->model->save($data))
		{
			...
			{
				// ajax request
				VRCHttpDocument::getInstance($app)->close($error->getCode() ?: 500, $string);
			}
			else
			{
				// post request, enqueue message and do redirect
		...
		if ($ajax)
		{
			...
			VRCHttpDocument::getInstance($app)->json(1);
		}
	...
	public function delete()
	{
		...
		{
			$app->enqueueMessage(__('1 item deleted.', 'vikrentcar'));
		}
	...
	public function dismissbc()
	{
		...
		VikRentCarInstaller::unregisterBreakingChanges($files);
