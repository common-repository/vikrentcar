...
class VikRentCarInstaller
{
	...
	{
		// create log trace
		$trace = '### VikRentCar Automatic Update | ' . JHtml::fetch('date', new JDate(), 'Y-m-d H:i:s') . "\n\n";
	...
	public static function getBreakingChanges()
	{
		...
		$files = get_option('vikrentcar_breaking_changes');
		// decode from JSON
	...
	public static function registerBreakingChanges(array $files)
	{
		if ($files)
		{
			...
			update_option('vikrentcar_breaking_changes', json_encode($existing));
		}
	}
		...
		if ($pagenow == 'plugins.php' && VikRentCarLicense::isExpired())
		{
			if (!JFactory::getApplication()->input->cookie->getInt('vrc_update_warning_hide', 0))
			{
				...
				<div class="notice is-dismissible notice-warning" id="vrc-update-warning">
					<p>
						<strong><?php echo JText::translate('VRCPROVEXPWARNUPD'); ?></strong>
					</p>
				</div>
